...
public class SparkOperationsPractice {
	...
	public static void main(String[] args) {
		...
		JavaRDD<String> xformedIris = irisRDD.map(s -> {
		...
		//First split as KV
		JavaPairRDD<String, Double[]> irisKV = irisRDD.mapToPair(arg0 -> {
	
			String[] attList = arg0.split(",");
			
			// Handle header line
			Double[] sepalLength = { (attList[0].equals("SepalLength") ? 0 : Double.valueOf(attList[0])), 1.0 };
			
			return new Tuple2<String, Double[]>(attList[4], sepalLength);
		});
		
		//Find sum of Sepal Length and Count by Key
		JavaPairRDD<String, Double[]> irisSumKV = irisKV.reduceByKey((Double[] arg0, Double[] arg1) -> {
			
			return new Double[]{arg0[0] + arg1[0], arg0[1] + arg1[1]};
		});
		...
		JavaRDD<String> irisOut = irisRDD.map(s -> {
